...
function downloadFullExamBookletPDF() {
    let bookletHTML = `
    <div id="pdf-booklet" style="font-family: 'Times New Roman', serif; font-size: 8.8pt; line-height: 1.3; color: #111; padding: 10px;">
        <div style="text-align: center; border: 1.2pt solid #002f55; background-color: #f4f7fa; padding: 10px; margin-bottom: 15px;">
            <h2 style="margin: 0 0 5px 0; font-size: 11pt; color: #002f55; text-transform: uppercase;">YABANCI DİL BİLGİSİ SEVİYE TESPİT SINAVI (YDS / e-YDS / YDT)</h2>
            <div style="font-size: 8.5pt; font-weight: bold; color: #333;">İNGİLİZCE TESTİ — 80 SORU | SÜRE: 180 DAKİKA</div>
        </div>
        <div style="column-count: 2; column-gap: 6mm; column-rule: 0.5pt solid #bbb;">
    `;

    let lastInst = "";
    questions.forEach((q, idx) => {
        const qid = q.id || (idx + 1);
        if (q.instruction && q.instruction !== lastInst) {
            bookletHTML += `<div style="background-color: #f8f9fa; border: 0.6pt solid #777; padding: 5px 7px; font-weight: bold; font-size: 8pt; margin: 8px 0; break-inside: avoid;">${q.instruction}</div>`;
            lastInst = q.instruction;
        }

        if (q.paragraph && (qid === 17 || qid === 22 || qid === 43 || qid === 47 || qid === 51 || qid === 55 || qid === 59)) {
            bookletHTML += `<div style="border: 0.6pt solid #555; background-color: #fafbfc; padding: 6px 8px; font-size: 8.3pt; line-height: 1.25; margin-bottom: 8px; break-inside: avoid; text-align: justify;"><strong>Paragraf:</strong><br>${q.paragraph}</div>`;
        }

        let questionText = q.text || `<b>${qid}. boşluğa uygun düşen sözcük ya da ifadeyi bulunuz.</b>`;
        if (questionText.includes('\n') && questionText.includes(':')) {
            questionText = questionText.split('\n').map(l => {
                if (l.includes(':')) {
                    const parts = l.split(':');
                    return `<div style="margin-bottom:2px;"><strong>${parts[0].trim()}:</strong> ${parts.slice(1).join(':').trim()}</div>`;
                }
                return `<div>${l}</div>`;
            }).join('');
        }
        ...
        <div style="margin-bottom: 10px; break-inside: avoid; page-break-inside: avoid;">
            <table style="width:100%; border-collapse:collapse; margin-bottom:3px;">
                <tr>
                    <td style="width:20px; font-weight:bold; font-size:9pt; vertical-align:top; padding:0;">${qid}.</td>
                    <td style="text-align:justify; vertical-align:top; padding:0;">${questionText}</td>
                </tr>
            </table>
            <table style="width:100%; border-collapse:collapse; margin-left:3px;">
        `;
        ...
        ['A', 'B', 'C', 'D', 'E'].forEach(letter => {
            if (q.options && q.options[letter]) {
                bookletHTML += `
                <tr>
                    <td style="width:18px; font-weight:bold; vertical-align:top; padding:1px 0;">${letter})</td>
                    <td style="text-align:justify; vertical-align:top; padding:1px 0;">${q.options[letter]}</td>
                </tr>`;
            }
        });

        bookletHTML += `</table></div>`;
    });

    bookletHTML += `</div></div>`;

    const container = document.createElement('div');
    container.innerHTML = bookletHTML;
    document.body.appendChild(container);

    const opt = {
        margin: [12, 10, 12, 10],
        filename: 'YDS_Ingilizce_80_Soru_Kitapcik.pdf',
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2 },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
        pagebreak: { mode: ['avoid-all', 'css', 'legacy'] }
